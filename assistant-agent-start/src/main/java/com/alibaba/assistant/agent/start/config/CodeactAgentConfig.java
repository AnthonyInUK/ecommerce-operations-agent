...
public class CodeactAgentConfig {
	...
	private static final String SYSTEM_PROMPT = """
			...
			- 框架优先：如果当前业务问题已经可以通过框架现有能力解决（如 Experience、FastIntent、Trigger、Reply、Memory、CodeactTool、PromptContributor），优先复用这些能力，不要重复造轮子
			- 边界透明：如果发现框架现有设计难以稳定解决当前问题，不要强行绕出一套平行体系，应明确指出这是框架能力边界，并说明缺口在哪里
			- 学习引导优先：当用户是在学习框架、源码或业务设计，而不是单纯索要答案时，优先用“提出小问题 -> 给 2~3 个猜想选项 -> 让用户先选 -> 再做最小源码验证/实验 -> 最后共同得出结论”的方式引导理解，不要直接灌输完整答案
			- 证据优先于结论：解释框架设计时，优先给出最少量源码入口、最小验证实验和业务含义，让用户形成自己的判断；避免把结论包装成需要死记硬背的标准答案
			...
			5. 框架能力优先级：
			   - 需要复用标准业务经验时，优先使用 Experience / read_exp / search_exp，而不是在代码里重新硬编码一套知识
			   - 需要高频问题快速命中时，优先考虑 FastIntent / verified cases，而不是额外造一层旁路逻辑
			   - 需要定时、异常触发、通知推送时，优先复用 Trigger / Reply 模块，而不是自己实现新的调度和消息分发
			   - 只有当框架原生能力无法稳定覆盖当前业务需求时，才允许补充最小自定义逻辑，并且需要明确标记这是临时补位，不是新的长期主链

			6. 学习型对话方式：
			   - 当用户要“读懂框架”时，不要一次性讲完；一次只推进一个小问题
			   - 先把问题翻译成更容易理解的业务/设计问题，再给 2~3 个候选猜想
			   - 优先让用户自己选判断，再进入源码验证
			   - 源码验证只给最少量入口（1~2 个关键方法/类），不要一上来铺满整个实现
			   - 验证完成后，最后再总结成“自己的话能怎么讲”，帮助用户形成面试表达
			</experience_usage>
	...
	public CodeactAgent grayscaleCodeactAgent(
            ...
            @Autowired(required = false) ResilientCallExecutor llmResilientCallExecutor,
		...
		ChatModel resilientModel = wrapWithResilience(chatModel, environment, llmResilientCallExecutor);
	 ...
	 */
	/**
	 * 用共享的韧性执行器（{@link ResilientCallExecutor} Bean，见 {@code LlmResilienceConfig}）包装 ChatModel。
	 * 执行器为 null 表示韧性层已关闭（{@code app.llm.resilience.enabled=false}），此时返回原始模型。
	 */
	private ChatModel wrapWithResilience(ChatModel chatModel, Environment environment, ResilientCallExecutor executor) {
		if (executor == null) {
			logger.info("CodeactAgentConfig#wrapWithResilience - reason=韧性层已关闭，使用原始 ChatModel");
			return chatModel;
		}
		String fallbackText = environment == null ? "系统当前繁忙，暂时无法完成本次分析，请稍后重试。"
				: environment.getProperty("app.llm.resilience.fallback-text",
						"系统当前繁忙，暂时无法完成本次分析，请稍后重试。");
		logger.info("CodeactAgentConfig#wrapWithResilience - reason=已启用 LLM 韧性层（共享执行器）");
